...
转换规则：
• ceil(累计 / 8) → 每 8 小时算 1 个工作日，不足 1 天按 1 天算（向上取整）
```
...
|------|------|------|------|
| id | string | ✓ | 唯一标识 |
| projectName | string | ✓ | 项目号 |
| projectTitle | string | - | 项目名称（可选，输入已知项目号自动识别） |
...
CSV/TSV 批量导入支持：
- **分隔符自动识别**：逗号、Tab、分号、竖线
- **日期格式自动识别**：`YYYY-MM-DD` `YYYY/MM/DD` `YYYY.MM.DD` `YYYY年MM月DD日` `MM/DD/YYYY` `DD.MM.YYYY`，统一转为 `YYYY-MM-DD`
- **向后兼容**：自动识别旧版（5列）和新版（6列含项目名称）格式
